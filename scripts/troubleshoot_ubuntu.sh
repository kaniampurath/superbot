...
redacted_env() {
  for key in RUN_MODE SYSTEM_STAGE SYMBOLS KLINE_INTERVAL UI_HOST_PORT MARKET_DATA_SOURCE BINANCE_REST_BASE_URL BINANCE_WS_BASE_URL MARKET_DATA_INTERVALS MARKET_DATA_WS_STALE_SECONDS MARKET_DATA_WS_RECONNECT_SECONDS MARKET_DATA_TRUST_ENV_PROXY BINANCE_VERIFY_TLS MYSQL_HOST MYSQL_PORT MYSQL_DATABASE MYSQL_USER REDIS_HOST REDIS_PORT ENABLE_REAL_TESTNET_ORDERS TRAINING_AUTO_APPROVE_PAPER DEPLOYMENT_UNLOCKED; do
    value="$(env_value "$key")"
    if [[ -n "$value" ]]; then
      printf "%s=%s\n" "$key" "$value"
    fi
